...
local function run(msg, matches) -- by TH3CZAR TEle _ @TH3CZAR
  ...
  for k, v in pairs( plugins_names( )) do  
      -- get the name 
      v = string.match (v, "(.*)%.lua") 
sendDocument(msg.to.id, "./plugins/"..v..".lua", msg.id, "💢¦ الملف مقدم من قناة قيصر توكن \n💢¦ تابع قناة السورس @CHTH3CZAR")
...
sendDocument(msg.to.id, "./plugins/"..file..".lua", msg.id, "💢¦ الملف مقدم من قناة قيصر توكن \n💢¦ تابع قناة السورس @CHTH3CZAR")
end
end
end
